...
namespace {/**
 * Nette Framework (version 2.0-dev bfb65e0 released on 2010-12-01)
 *
...
isLinkCurrent($destination,$args=array()){if(!is_array($args)){$args=func_get_args();array_shift($args);}$this->link($destination,$args);return$this->getPresenter()->getLastCreatedRequestFlag('current');}function
...
as$id=>$foo){$routes[$id][]=$route;}}}$this->cachedRoutes=$routes;}$presenter=strtolower($appRequest->getPresenterName());if($this->module){if(strncasecmp($presenter,$this->module,strlen($this->module))===0){$appRequest=clone$appRequest;$appRequest->setPresenterName(substr($appRequest->getPresenterName(),strlen($this->module)));}else{return
NULL;}}if(!isset($this->cachedRoutes[$presenter]))$presenter='*';foreach($this->cachedRoutes[$presenter]as$route){$uri=$route->constructUrl($appRequest,$refUri);if($uri!==NULL){return$uri;}}return
NULL;}function
...
new\InvalidStateException("Cannot open journal file '$this->file'.");}if(!($this->handle=@fopen($this->file,'rb'))){$this->handle=NULL;}else{list(,$magic,$sectionCount)=unpack('N2',fread($this->handle,2*self::INT32));if($magic!==self::MAGIC){fclose($this->handle);throw
...
new\InvalidStateException("Cannot open logfile '$logfile' for journal.");}$doMergeFirst=FALSE;$openNewLog=FALSE;$reopen=FALSE;if(flock($this->logHandle,LOCK_SH|LOCK_NB)){if(file_exists($logfile=$this->file.self::EXTLOGNEW)){if(($logmtime=@filemtime($this->file.self::EXTLOG))===FALSE){throw
...
new\InvalidStateException("Cannot open logfile '$logfile'.");}if(!flock($this->logHandle,LOCK_SH)){throw
new\InvalidStateException('Cannot acquite shared lock on log.');}}if($openNewLog){fclose($this->logHandle);if(!($this->logHandle=@fopen($logfile=$this->file.self::EXTLOGNEW,'a+b'))){throw
new\InvalidStateException("Cannot open logfile '$logfile'.");}$this->isLogNew=TRUE;}$this->logMerge=$this->mergeLogFile($this->logHandle,0,$this->logMerge);$this->logMergeP=ftell($this->logHandle);if($this->logMergeP===0){if(!flock($this->logHandle,LOCK_EX)){throw
new\InvalidStateException('Cannot acquite exclusive lock on log.');}$data=serialize(array());$data=pack('N',strlen($data)).$data;$written=fwrite($this->logHandle,$data);if($written===FALSE||$written!==strlen($data)){throw
new\InvalidStateException('Cannot write empty packet to log.');}if(!flock($this->logHandle,LOCK_SH)){throw
new\InvalidStateException('Cannot acquite shared lock on log.');}}}function
...
new\InvalidStateException('Cannot write to log.');}if(!$this->isLogNew){fseek($this->logHandle,0,SEEK_END);$size=ftell($this->logHandle);if($size>self::LOGMAXSIZE){$this->rebuild();}}return
TRUE;}private
function
...
new\InvalidStateException("Cannot reopen logfile '$logfile'.");}$this->logMerge=array();$this->logMergeP=0;if(!($this->handle=@fopen($this->file,'rb'))){throw
new\InvalidStateException("Cannot reopen file '$this->file'.");}clearstatcache();$this->mtime=(int)@filemtime($this->file);$this->sections=$newsections;return
TRUE;}private
function
...
__construct($dir,Nette\Context$context=NULL){if(self::$useDirectories===NULL){$uniq=uniqid('_',TRUE);umask(0000);if(!@mkdir("$dir/$uniq",0777)){throw
...
__construct($host='localhost',$port=11211,$prefix='',Nette\Context$context=NULL){if(!self::isAvailable()){throw
new\NotSupportedException("PHP extension 'memcache' is not loaded.");}$this->prefix=$prefix;$this->context=$context;$this->memcache=new\Memcache;Nette\Debug::tryError();$this->memcache->connect($host,$port);if(Nette\Debug::catchError($e)){throw
...
new\NotSupportedException("SQLite or SQLite3 extension is required for storing tags and priorities.");}$this->database=extension_loaded('sqlite')?new
...
sqlite_escape_string($s);}}}}namespace Nette{use
Nette;class
...
Context;$context->addService('Nette\\Caching\\ICacheJournal',Environment::getContext());$dir=Environment::getVariable('tempDir').'/cache';umask(0000);@mkdir($dir,0777);return
new
Nette\Caching\FileStorage($dir,$context);}static
function
...
function
addClass($class,$file,$time){$lClass=strtolower($class);if(isset($this->list[$lClass][0])&&$this->list[$lClass][0]!==$file&&is_file($this->list[$lClass][0])){$e=new\InvalidStateException("Ambiguous class '$class' resolution; defined in $file and in ".$this->list[$lClass][0].".");{throw$e;}}$this->list[$lClass]=array($file,$time,$class);}private
function
...
instanceof
TemplateCacheStorage){$storage->hint=str_replace(dirname(dirname($this->file)),'',$this->file);}$cached=$content=$cache[$this->file];if($content===NULL){try{$content=$this->compile(file_get_contents($this->file));$content="<?php\n\n// source file: $this->file\n\n?>$content";}catch(TemplateException$e){$e->setSourceFile($this->file);throw$e;}$cache->save($this->file,$content,array(Cache::FILES=>$this->file,Cache::EXPIRATION=>self::$cacheExpire,Cache::CONSTS=>'Nette\Framework::REVISION'));$cache->release();$cached=$cache[$this->file];}if($cached!==NULL&&$storage
instanceof
...
initialize($filter,&$s){$this->filter=$filter;$this->nodes=array();$this->blocks=array();$this->namedBlocks=array();$this->extends=NULL;$this->uniq=substr(md5(uniqid('',TRUE)),0,10);$this->cacheCounter=0;$filter->context=LatteFilter::CONTEXT_TEXT;$filter->escape='Nette\Templates\TemplateHelpers::escapeHtml';}function
...
macroForeach($content){return'$iterator = $_l->its[] = new Nette\SmartCachingIterator('.preg_replace('# +as +#i',') as ',$this->formatMacroArgs($content),1);}function
...
REVISION='bfb65e0 released on 2010-12-01';public
...
new\InvalidStateException('Critical section has not been initialized.');}fclose(self::$criticalSections);self::$criticalSections=NULL;}}}namespace Nette\Web{use
Nette;class
Html
extends
Nette\Object
...
function
getSession(){return
Environment::getSession();}}}namespace{
class_alias('Nette\AmbiguousServiceException', 'NAmbiguousServiceException');
class_alias('Nette\Application\AbortException', 'NAbortException');
class_alias('Nette\Application\AppForm', 'NAppForm');
class_alias('Nette\Application\Application', 'NApplication');
class_alias('Nette\Application\ApplicationException', 'NApplicationException');
class_alias('Nette\Application\BadRequestException', 'NBadRequestException');
class_alias('Nette\Application\BadSignalException', 'NBadSignalException');
class_alias('Nette\Application\CliRouter', 'NCliRouter');
class_alias('Nette\Application\Control', 'NControl');
class_alias('Nette\Application\DownloadResponse', 'NDownloadResponse');
class_alias('Nette\Application\ForbiddenRequestException', 'NForbiddenRequestException');
class_alias('Nette\Application\ForwardingResponse', 'NForwardingResponse');
class_alias('Nette\Application\IPartiallyRenderable', 'NIPartiallyRenderable');
class_alias('Nette\Application\IPresenter', 'NIPresenter');
class_alias('Nette\Application\IPresenterLoader', 'NIPresenterLoader');
class_alias('Nette\Application\IPresenterResponse', 'NIPresenterResponse');
class_alias('Nette\Application\IRenderable', 'NIRenderable');
class_alias('Nette\Application\IRouter', 'NIRouter');
class_alias('Nette\Application\ISignalReceiver', 'NISignalReceiver');
class_alias('Nette\Application\IStatePersistent', 'NIStatePersistent');
class_alias('Nette\Application\InvalidLinkException', 'NInvalidLinkException');
class_alias('Nette\Application\InvalidPresenterException', 'NInvalidPresenterException');
class_alias('Nette\Application\JsonResponse', 'NJsonResponse');
class_alias('Nette\Application\Link', 'NLink');
class_alias('Nette\Application\MultiRouter', 'NMultiRouter');
class_alias('Nette\Application\Presenter', 'NPresenter');
class_alias('Nette\Application\PresenterComponent', 'NPresenterComponent');
class_alias('Nette\Application\PresenterComponentReflection', 'NPresenterComponentReflection');
class_alias('Nette\Application\PresenterLoader', 'NPresenterLoader');
class_alias('Nette\Application\PresenterRequest', 'NPresenterRequest');
class_alias('Nette\Application\RedirectingResponse', 'NRedirectingResponse');
class_alias('Nette\Application\RenderResponse', 'NRenderResponse');
class_alias('Nette\Application\Route', 'NRoute');
class_alias('Nette\Application\RoutingDebugger', 'NRoutingDebugger');
class_alias('Nette\Application\SimpleRouter', 'NSimpleRouter');
class_alias('Nette\ArrayList', 'NArrayList');
class_alias('Nette\ArrayTools', 'NArrayTools');
class_alias('Nette\Caching\Cache', 'NCache');
class_alias('Nette\Caching\DummyStorage', 'NDummyStorage');
class_alias('Nette\Caching\FileJournal', 'NFileJournal');
class_alias('Nette\Caching\FileStorage', 'NFileStorage');
class_alias('Nette\Caching\ICacheJournal', 'NICacheJournal');
class_alias('Nette\Caching\ICacheStorage', 'NICacheStorage');
class_alias('Nette\Caching\MemcachedStorage', 'NMemcachedStorage');
class_alias('Nette\Caching\MemoryStorage', 'NMemoryStorage');
class_alias('Nette\Caching\SqliteJournal', 'NSqliteJournal');
class_alias('Nette\Callback', 'NCallback');
class_alias('Nette\CallbackFilterIterator', 'NCallbackFilterIterator');
class_alias('Nette\Component', 'NComponent');
class_alias('Nette\ComponentContainer', 'NComponentContainer');
class_alias('Nette\Config\Config', 'NConfig');
class_alias('Nette\Config\ConfigAdapterIni', 'NConfigAdapterIni');
class_alias('Nette\Config\IConfigAdapter', 'NIConfigAdapter');
class_alias('Nette\Configurator', 'NConfigurator');
class_alias('Nette\Context', 'NContext');
class_alias('Nette\Debug', 'NDebug');
class_alias('Nette\DebugHelpers', 'NDebugHelpers');
class_alias('Nette\DebugPanel', 'NDebugPanel');
class_alias('Nette\Environment', 'NEnvironment');
class_alias('Nette\Finder', 'NFinder');
class_alias('Nette\Forms\Button', 'NButton');
class_alias('Nette\Forms\Checkbox', 'NCheckbox');
class_alias('Nette\Forms\DefaultFormRenderer', 'NDefaultFormRenderer');
class_alias('Nette\Forms\FileUpload', 'NFileUpload');
class_alias('Nette\Forms\Form', 'NForm');
class_alias('Nette\Forms\FormContainer', 'NFormContainer');
class_alias('Nette\Forms\FormControl', 'NFormControl');
class_alias('Nette\Forms\FormGroup', 'NFormGroup');
class_alias('Nette\Forms\HiddenField', 'NHiddenField');
class_alias('Nette\Forms\IFormControl', 'NIFormControl');
class_alias('Nette\Forms\IFormRenderer', 'NIFormRenderer');
class_alias('Nette\Forms\ISubmitterControl', 'NISubmitterControl');
class_alias('Nette\Forms\ImageButton', 'NImageButton');
class_alias('Nette\Forms\MultiSelectBox', 'NMultiSelectBox');
class_alias('Nette\Forms\RadioList', 'NRadioList');
class_alias('Nette\Forms\Rule', 'NRule');
class_alias('Nette\Forms\Rules', 'NRules');
class_alias('Nette\Forms\SelectBox', 'NSelectBox');
class_alias('Nette\Forms\SubmitButton', 'NSubmitButton');
class_alias('Nette\Forms\TextArea', 'NTextArea');
class_alias('Nette\Forms\TextBase', 'NTextBase');
class_alias('Nette\Forms\TextInput', 'NTextInput');
class_alias('Nette\Framework', 'NFramework');
class_alias('Nette\FreezableObject', 'NFreezableObject');
class_alias('Nette\GenericRecursiveIterator', 'NGenericRecursiveIterator');
class_alias('Nette\IComponent', 'NIComponent');
class_alias('Nette\IComponentContainer', 'NIComponentContainer');
class_alias('Nette\IContext', 'NIContext');
class_alias('Nette\IDebugPanel', 'NIDebugPanel');
class_alias('Nette\IFreezable', 'NIFreezable');
class_alias('Nette\ITranslator', 'NITranslator');
class_alias('Nette\Image', 'NImage');
class_alias('Nette\ImageMagick', 'NImageMagick');
class_alias('Nette\InstanceFilterIterator', 'NInstanceFilterIterator');
class_alias('Nette\Json', 'NJson');
class_alias('Nette\JsonException', 'NJsonException');
class_alias('Nette\Loaders\AutoLoader', 'NAutoLoader');
class_alias('Nette\Loaders\LimitedScope', 'NLimitedScope');
class_alias('Nette\Loaders\RobotLoader', 'NRobotLoader');
class_alias('Nette\Mail\IMailer', 'NIMailer');
class_alias('Nette\Mail\Mail', 'NMail');
class_alias('Nette\Mail\MailMimePart', 'NMailMimePart');
class_alias('Nette\Mail\SendmailMailer', 'NSendmailMailer');
class_alias('Nette\Mail\SmtpException', 'NSmtpException');
class_alias('Nette\Mail\SmtpMailer', 'NSmtpMailer');
class_alias('Nette\MapIterator', 'NMapIterator');
class_alias('Nette\NeonException', 'NNeonException');
class_alias('Nette\NeonParser', 'NNeonParser');
class_alias('Nette\Object', 'NObject');
class_alias('Nette\ObjectMixin', 'NObjectMixin');
class_alias('Nette\Paginator', 'NPaginator');
class_alias('Nette\RecursiveCallbackFilterIterator', 'NRecursiveCallbackFilterIterator');
class_alias('Nette\RecursiveComponentIterator', 'NRecursiveComponentIterator');
class_alias('Nette\Reflection\Annotation', 'NAnnotation');
class_alias('Nette\Reflection\AnnotationsParser', 'NAnnotationsParser');
class_alias('Nette\Reflection\ClassReflection', 'NClassReflection');
class_alias('Nette\Reflection\ExtensionReflection', 'NExtensionReflection');
class_alias('Nette\Reflection\FunctionReflection', 'NFunctionReflection');
class_alias('Nette\Reflection\IAnnotation', 'NIAnnotation');
class_alias('Nette\Reflection\MethodReflection', 'NMethodReflection');
class_alias('Nette\Reflection\ParameterReflection', 'NParameterReflection');
class_alias('Nette\Reflection\PropertyReflection', 'NPropertyReflection');
class_alias('Nette\RegexpException', 'NRegexpException');
class_alias('Nette\SafeStream', 'NSafeStream');
class_alias('Nette\Security\AuthenticationException', 'NAuthenticationException');
class_alias('Nette\Security\IAuthenticator', 'NIAuthenticator');
class_alias('Nette\Security\IAuthorizator', 'NIAuthorizator');
class_alias('Nette\Security\IIdentity', 'NIIdentity');
class_alias('Nette\Security\IResource', 'NIResource');
class_alias('Nette\Security\IRole', 'NIRole');
class_alias('Nette\Security\Identity', 'NIdentity');
class_alias('Nette\Security\Permission', 'NPermission');
class_alias('Nette\Security\SimpleAuthenticator', 'NSimpleAuthenticator');
class_alias('Nette\SmartCachingIterator', 'NSmartCachingIterator');
class_alias('Nette\String', 'NString');
class_alias('Nette\Templates\CachingHelper', 'NCachingHelper');
class_alias('Nette\Templates\FileTemplate', 'NFileTemplate');
class_alias('Nette\Templates\IFileTemplate', 'NIFileTemplate');
class_alias('Nette\Templates\ITemplate', 'NITemplate');
class_alias('Nette\Templates\LatteException', 'NLatteException');
class_alias('Nette\Templates\LatteFilter', 'NLatteFilter');
class_alias('Nette\Templates\LatteMacros', 'NLatteMacros');
class_alias('Nette\Templates\Template', 'NTemplate');
class_alias('Nette\Templates\TemplateCacheStorage', 'NTemplateCacheStorage');
class_alias('Nette\Templates\TemplateException', 'NTemplateException');
class_alias('Nette\Templates\TemplateFilters', 'NTemplateFilters');
class_alias('Nette\Templates\TemplateHelpers', 'NTemplateHelpers');
class_alias('Nette\Tokenizer', 'NTokenizer');
class_alias('Nette\TokenizerException', 'NTokenizerException');
class_alias('Nette\Tools', 'NTools');
class_alias('Nette\Web\Html', 'NHtml');
class_alias('Nette\Web\HttpContext', 'NHttpContext');
class_alias('Nette\Web\HttpRequest', 'NHttpRequest');
class_alias('Nette\Web\HttpRequestFactory', 'NHttpRequestFactory');
class_alias('Nette\Web\HttpResponse', 'NHttpResponse');
class_alias('Nette\Web\HttpUploadedFile', 'NHttpUploadedFile');
class_alias('Nette\Web\IHttpRequest', 'NIHttpRequest');
class_alias('Nette\Web\IHttpResponse', 'NIHttpResponse');
class_alias('Nette\Web\IUser', 'NIUser');
class_alias('Nette\Web\Session', 'NSession');
class_alias('Nette\Web\SessionNamespace', 'NSessionNamespace');
class_alias('Nette\Web\Uri', 'NUri');
class_alias('Nette\Web\UriScript', 'NUriScript');
class_alias('Nette\Web\User', 'NUser');}
